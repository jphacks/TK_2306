...
class Optimize:
    def __init__(self,days:int,workers:int, #days -> 日数 workers -> 従事者の人数
                candidate:list[list[list[list[int]]]], # candidate -> 解の候補のリスト、candidate[i][j][k] -> i番目の解の候補において、j日目のk時間目に働いている従事者のリスト
                ...
                attribute_requirement:list[list[tuple[int,int]]], # (m,M)=attribute_requirement[48*i+j][k] で i 日目の j 時間目に役職 k がいるべき人数が m 人以上 M 人以下
