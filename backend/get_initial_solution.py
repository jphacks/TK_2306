...
class State:
    def __init__(
        ...
        self.number_of_people_per_time: list[list[int]] = [
            [0 for _ in range(48)] for _ in range(days)
        ]
        self.number_of_people_per_time_per_attribute: list[list[list[int]]] = [
            [[0 for _ in range(number_of_attributes)] for _ in range(48)]
            for _ in range(days)
        ]
                ...
                for from_, to_ in shift_preference_per_person[person_id][day]:
                    for time_ in range(from_, to_):
                        ...
                        for attribute in attributes_per_person[person_id]:
                            self.number_of_people_per_time_per_attribute[day][time_][
                                attribute
                            ] += 1
    ...
    def is_ok(
        ...
        for day in range(self.days):
            for time_, attribute_requirement in enumerate(
                attribute_requirement_per_time[day]
            ):
                for attribute_id, (min_, max_) in enumerate(attribute_requirement):
                    if (
                        ...
                        < min_
                        or self.number_of_people_per_time_per_attribute[day][time_][
                            attribute_id
                        ]
                        > max_
                    ):
...
def get_initial_solutions(
    ...
    assert len(shift_preferences_per_person[0]) == number_of_days
    assert len(shift_requirements_per_time) == number_of_days
    assert len(shift_requirements_per_time[0]) == 48
    assert len(attributes_per_person) == number_of_people
    assert len(attribute_requirements_per_time) == number_of_days
    assert len(attribute_requirements_per_time[0]) == 48
    assert len(attribute_requirements_per_time[0][0]) == number_of_attributes

    start_time = time.perf_counter()
    ret = []

    while time.perf_counter() - start_time < limit_time_sec:
        cur_solution: list[list[list[tuple[int, int]]]] = [
            [[] for _ in range(number_of_days)] for _ in range(number_of_people)
        ]
        ...
        while True:
            ...
            rnd = random.randint(
                0, len(shift_preferences_per_person[rnd_person][rnd_day]) * 2 - 1
            )
            if (
            ...
            if cur_state.delete(
                ...
                if debug:
                    ...
                    print(
                        f"attributes_requirements = {attribute_requirements_per_time}"
                    )
                    print(f"cur_solution = {cur_solution}")
                (cur_time_start, cur_time_end) = cur_solution[rnd_person][rnd_day][
                    rnd // 2
                ]
                if rnd % 2 == 0:
                    cur_solution[rnd_person][rnd_day][rnd // 2] = (
                        delete_time_end,
                        cur_time_end,
                    )
                else:
