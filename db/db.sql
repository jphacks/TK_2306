...
CREATE TABLE event_attributes(id integer primary key, date_id integer, name text, max_people integer, min_people integer, foreign key (date_id) references events(id));
...
CREATE TABLE shift(group_id text, user_id integer, time_from text, time_to text, foreign key (user_id) references users(id), foreign key (group_id) references events(group_id));
