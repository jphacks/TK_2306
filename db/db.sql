...
CREATE TABLE events(group_id integer primary key, name text, admin_id integer);
CREATE TABLE event_dates(id integer primary key, group_id integer, date text, time_from text, time_to text, max_people integer, min_people integer, foreign key (group_id) references events(group_id));
...
CREATE TABLE users(id integer primary key, name text, group_id integer, foreign key (group_id) references events(group_id));
